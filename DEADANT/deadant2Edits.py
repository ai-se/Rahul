...
import sys, random; import pdb, math, numpy as np
...
def fonseca():
  def f1(x): return (1-exp**np.sum([(x[z]-1/(len(x)**0.5)) 
                                      for z in xrange(len(x))]))
  def f2(x): return (1-exp**np.sum([(x[z]+1/((len(x))**0.5)) 
                                      for z in xrange(len(x))]))
  return Cols(fonseca,
                 [N(least=-4, most=4)
                 , O(f=f1)
