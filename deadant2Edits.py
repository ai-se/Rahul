...
def decimateClose(n=10000,p=2,rseed=None):
...
class N(Col):
  ...
  def any(self):
    return self.least + rand()*(self.most - self.least)
  def __iadd__(self,x):
    assert x >= self.least and x <= self.most
    self.lo = min(self.lo,x)
    self.hi = max(self.hself,x)
    return self
  def norm(self,x):
...
class O(Col):
  ...
  def score(self,lst):
    ...
    return x
  def height(self):
    return self.n.norm(self._score)
  def better(self,x,y):
...
class Meta(Col):
  ...
  def fuse(self,x,w1,y,w2): 
    ...
    tmp.weight = w1+w2
    return tmp
  def nudge(self): return self.any()
  def __repr__(self):
    return self.of.name + ':' \
...
class Cols:
  ...
  def nudge(self,lst1,lst2):
    return [one.nudge(x,y) # removed w1, w2 from this line. 
  ...
  def dist(self,lst1,lst2,peeking=False):
    ...
    d= total**0.5/c**0.5
    if not peeking: self.cl += d   #cl -> self.cl       
    return d
